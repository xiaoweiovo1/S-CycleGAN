...
def draw(axis_x,axis_y,value_y):
    axis_x = list(range(1,axis_x+1))
    ...
    plt.title(value_y, fontsize=24)
    # 给x轴添加标签，设置字体大小
    plt.xlabel("Epoch", fontsize=14)
    # 给y轴添加标签，设置字体大小
    plt.ylabel(value_y, fontsize=14)
    # 设置每个坐标轴的取值范围
    plt.axis([0, len(axis_x), 0, 100])  # [x.x,x.y,y.x,y.y]
    # tick_params()设置刻度标记的大小，设置刻度的样式
    plt.tick_params(axis='both', labelsize=14)
    # 打开matplotlib查看器，并显示绘制的图形
    plt.savefig(value_y+".png", dpi=300)
...
def train(cgan_net,max_img,batch_size,trainA,trainB,lr_rate,shape,pool_size,model_dir):
    ...
    with tf.Session() as sess:
        ...
        while epoch < 51:
            ...
            if epoch >= 100:
                lr_rate = lr_init - ((epoch-100)*lr_init)/100
            
            for step in range(max_img / batch_size):
                TP = 0
                FP = 0
                FN = 0
                if countA >= lenA:
        ...
        axis_x = 50
        draw(axis_x,F1,'F1')
        draw(axis_x,mIOU,'mIOU')
        draw(axis_x,Time,'time')
        draw(axis_x,loss1,'discA_loss')
        draw(axis_x,loss2,'genA_loss')
        draw(axis_x,loss3,'discB_loss')
        draw(axis_x,loss4,'genB_loss')
