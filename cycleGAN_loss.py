...
class CycleGAN:
    ...
    def model_loss(self,real_disc_A,real_disc_B,fake_disc_A,fake_disc_B,fake_pool_disc_A,fake_pool_disc_B,input_A,cyclicA,input_B,cyclicB,loss_type,g_window,norm_const):
        ...
        if loss_type == "vgg":
          vgg = Vgg19()
          [w, h, d] = input_A.get_shape().as_list()[1:]
          cyc_A = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((vgg.extract_feature(input_A) -  vgg.extract_feature(cyclicA))))) / (w*h*d))
          [w, h, d] = input_B.get_shape().as_list()[1:]
          cyc_B = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((vgg.extract_feature(input_B) -  vgg.extract_feature(cyclicB))))) / (w*h*d))
